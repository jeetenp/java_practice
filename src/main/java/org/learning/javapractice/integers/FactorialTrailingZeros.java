...
public class FactorialTrailingZeros {
    public static void main(String[] args) {
        ...
        int n = 15;
        // int count = factorialTrailingZeros.getFactorialTrailingZeros(n);
        int count = factorialTrailingZeros.trailingZeroes(n);
        System.out.println(count);
    }

    public int getFactorialTrailingZeros(int n) {
        Factorial factorial = new Factorial();
        long fact = factorial.getFactorial(n);
        long f = fact;
        int count = 0;
        while (fact > 0) {
            if (fact % 10 == 0) {
        ...
        System.out.println(String.format("Factorial of %s = %s", n, f));
...
Instead of keeping track of the number of 2s and 5s, we really just need to keep track of the number of 5s.
This is because in any factorial calculation there are always going to be more multiples of 2 than 5.
